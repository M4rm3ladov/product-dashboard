...
<body>
    ...
    <div>
        ...
        <form action="/edit/products/validate/<?= $product['id']; ?>" method="post">
<?php
...
?>
            ...
            <p>
                ...
                <input type="text" name="name" value="<?= isset($this->session->flashdata('values')['name']) ? $this->session->flashdata('values')['name'] : $product['name'];?>" id="name">
                <p class="error"><?= isset($this->session->flashdata('errors')['name']) ? $this->session->flashdata('errors')['name'] : ''; ?></p>
            </p>
            <p>
                <label for="description">Description:</label>
                <textarea name="description" id="description"><?= isset($this->session->flashdata('values')['description']) ? $this->session->flashdata('values')['description'] : $product['description'];?></textarea>
                <p class="error"><?= isset($this->session->flashdata('errors')['description']) ? $this->session->flashdata('errors')['description'] : ''; ?></p>
            </p>
            <p>
                <label for="price">Price:</label>
                <input type="number" name="price" value="<?= isset($this->session->flashdata('values')['price']) ? $this->session->flashdata('values')['price'] : $product['price'];?>" placeholder="0.00" min="1" step="any" id="price">
            ...
            <p>
                ...
                <input type="number" name="count" value="<?= isset($this->session->flashdata('values')['count']) ? $this->session->flashdata('values')['count'] : $product['product_count'];?>" placeholder="0" min="1" max="" id="count">
